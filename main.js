...
window.addEventListener("DOMContentLoaded", (e) => {
  ...
  let defaultLang = langOptions.filter(
...
function handleDom() {
  ...
  document
    ...
    .insertAdjacentHTML(
      "beforeend",
      "<br><a download='my_qr_code.png' href='" +
        dataURL +
        "'>Download QR code</a> | "
    );
  document
    ...
    .insertAdjacentHTML(
      "beforeend",
      "<a style='cursor:pointer' onClick='printAsPDF()'>Print As PDF</a> | "
    );
  ...
  document
    ...
    .insertAdjacentHTML(
      ...
      "<br><div style='display:flex'><input type='text' value='" +
        encodedString +
        "' id='to_copy' readonly><i class='fa fa-copy icon' onclick='copyLink()'></i></div>"
    );
...
const printAsPDF = () => {
  printJS({
    printable: dataURL,
    type: "image",
    header: `QR code of ${form.children[0].children[0][0].value}`,
  });
};
...
// Adds the sharer functionality

const sharebtn = document.querySelector(".share-btn");

for(let [key,value] of Object.entries(sharebtn.children)) {
  // console.log(value);
  if(!value.dataset.url) {
